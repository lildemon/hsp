...
var eco = require('eco');
var wrapmod = require('../assets/wrapmodule');
...
function getAst(inputFile, charset) {
  ...
  var source = fs.readFileSync(inputFile, charset || 'utf8');
  if(path.extname(inputFile) === '.coffee') {
	source = wrapmod({code:Coffee.compile(source.toString())});
  }
